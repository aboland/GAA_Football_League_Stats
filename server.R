...
shinyServer(function(input, output) {
  ...
  y_numerator <- reactive({
    ...
    if(input$stat_choice_y == "goals"){
      ...
      for(i in 1:nrow(output)){
        ...
        }else if(input$home_away_y=="home"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Team_Goals"])
        }else if(input$home_away_y=="away"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Opp_Goals"])
        }
      }
      ...
      for(i in 1:nrow(output)){
        if(input$home_away_y=="all"){
          ...
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Goals"])
        }
      }
      ...
      for(i in 1:nrow(output)){
        if(input$home_away_y=="all"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Opp_Points"]) + 
            sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Points"])
        }else if(input$home_away_y=="home"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Opp_Points"])
        }else if(input$home_away_y=="away"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Points"])
        }
      }
    ...
    if(input$stat_choice_y_per == "no_div"){
      ...
      for(i in 1:nrow(output)){
        if(input$home_away_y=="all"){
          ...
          output[i,"stat"] <- sum(gaa_date()$Team_Name==output[i,"team"])
        }else if(input$home_away_y=="away"){
          output[i,"stat"] <- sum(gaa_date()$Opp_Name==output[i,"team"])
        }
      }
  ...
  x_numerator <- reactive({
    ...
    if(input$stat_choice_x == "goals"){
      ...
      for(i in 1:nrow(output)){
        if(input$home_away_x=="all"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Team_Goals"]) + 
            sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Opp_Goals"])
        }else if(input$home_away_x=="home"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Team_Goals"])
        }else if(input$home_away_x=="away"){
          output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Opp_Goals"])
        }
      }
  ...
  x_denominator <- reactive({
    ...
    if(input$stat_choice_x_per == "no_div"){
      ...
      main_x_per <<- "per game"
      for(i in 1:nrow(output)){
        if(input$home_away_x=="all"){  output[i,"stat"] <- sum(gaa_date()$Team_Name==output[i,"team"]) + sum(gaa_date()$Opp_Name==output[i,"team"])
        }else if(input$home_away_x=="home"){  output[i,"stat"] <- sum(gaa_date()$Team_Name==output[i,"team"])
        }else if(input$home_away_x=="away"){  output[i,"stat"] <- sum(gaa_date()$Opp_Name==output[i,"team"])
        }
      }
    }else if(input$stat_choice_x_per == "p_goal_score"){
      main_x_per <<- "per goal scored"
      for(i in 1:nrow(output)){
        if(input$home_away_x=="all"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Team_Goals"]) + 
            sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Opp_Goals"])
        }else if(input$home_away_x=="home"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Team_Goals"])
        }else if(input$home_away_x=="away"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Opp_Goals"])
        }
      }
    }else if(input$stat_choice_x_per == "p_goal_conc"){
      main_x_per <<- "per goal conceded"
      for(i in 1:nrow(output)){
        if(input$home_away_x=="all"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Opp_Goals"]) + 
            sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Goals"])
        }else if(input$home_away_x=="home"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Opp_Goals"])
        }else if(input$home_away_x=="away"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Goals"])
        }
      }
    }else if(input$stat_choice_x == "p_points"){
      main_x_per <<- "per point scored"
      for(i in 1:nrow(output)){
        if(input$home_away_x=="all"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Opp_Points"]) + 
            sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Points"])
        }else if(input$home_away_x=="home"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Team_Name==output[i,"team"]), "Opp_Points"])
        }else if(input$home_away_x=="away"){  output[i,"stat"] <- sum(gaa_date()[which(gaa_date()$Opp_Name==output[i,"team"]), "Team_Points"])
        }
      }
  ...
  output$plot_stats_custom <- renderPlot({
    ...
    if(input$home_away_y=="all"){  y_lab_ha <<- NULL
    }else if(input$home_away_y=="home"){  y_lab_ha <<- "(home)"
    }else if(input$home_away_y=="away"){  y_lab_ha <<- "(away)"}
    if(input$home_away_x=="all"){  x_lab_ha <<- NULL
    }else if(input$home_away_x=="home"){  x_lab_ha <<- "(home)"
    }else if(input$home_away_x=="away"){  x_lab_ha <<- "(away)"}
  ...
  observe({
    if(!is.null(input$custom_plot_click$x)&&!is.null(input$custom_plot_click$y)&&!is.null(click_xdata)){
      ...
      output$info_cus <- renderText({
        paste0(team_colours()$team_neat[closest_team],":\ ",click_xlab,": ", click_xdata[closest_team],",\ ",click_ylab,": ", click_ydata[closest_team])
      })
    }else{
      output$info_cus <- renderText({
